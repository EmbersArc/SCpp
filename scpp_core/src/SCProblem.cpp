    ...
    discretization_data_t &dd)
{
    ...
    for (size_t k = 0; k < K; k++)
    {
        ...
        cvx::VectorX norm2_terms(v_X.rows() + 1);
        norm2_terms << cvx::par(0.5) + cvx::par(-0.5) * v_delta(k),
                                       cvx::dynpar(td.X.at(k)) - v_X.col(k);

        if (dd.interpolatedInput() or (not dd.interpolatedInput() and k < K - 1))
        {
            norm2_terms.conservativeResize(v_X.rows() + v_U.rows() + 1);
            norm2_terms.tail(v_U.rows()) = cvx::dynpar(td.U.at(k)) - v_U.col(k);
        }

        socp.addConstraint(cvx::lessThan(norm2_terms.norm(), cvx::par(0.5) + cvx::par(0.5) * v_delta(k)));
    }
